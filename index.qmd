...
:::{.callout-important}
### This affects the behavior of tools & features!
:::
...
### Double-click an image to open it in the `viewer` on the right
...
- Polyline (`v`) can be used like a brush by click-and-drag

## Annotation tool tips and tricks

- Annotations are editable, unless you lock them (Right-click ‣ Lock)
- You can use the Brush or Wand tool to refine Polygons (or brushed regions)
    - ensure that the Annotation is selected (yellow, by double-clicking with Move tool)
...
![](images/annotations_classes_menu.png){fig-alt="Screenshot of QuPath  application  showing the Annotations tab of the Analysis Pane with the Classes submenu shown in red." fig-align="center"}
...
- Presents the relationship between objects:
    - Parent > Child
- If you have Annotations within an Annotation, you can use:
    - Objects ‣ Annotations… ‣ Resolve hierarchy 
        - QuPath will attempt to place all child objects with parent objects 
